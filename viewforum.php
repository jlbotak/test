...
<?php echo $post_link ?>
	...
	<?php if ($pun_user["g_id"] == 1 || $pun_user["g_id"] == 2) {?>
		<p><strong><a href="moderate.php?fid=<?php echo $id ?>">Moderate forum</a></strong></p>
  <?php }?>
</div>
...
if ($db->num_rows($result))
{
	...
	while ($cur_topic = $db->fetch_assoc($result))
	{
		...
		if (!$pun_user['guest'] && $cur_topic['last_post'] > $pun_user['last_visit'] && (!isset($tracked_topics['topics'][$cur_topic['id']]) || $tracked_topics['topics'][$cur_topic['id']] < $cur_topic['last_post']) && (!isset($tracked_topics['forums'][$id]) || $tracked_topics['forums'][$id] < $cur_topic['last_post']) && is_null($cur_topic['moved_to']))
		{
			$item_status .= ' inew';
