	...
	$hisid=$id;
		while($max--) 
			...
			if (isset($_POST['beta']) && $_POST['beta'] == "hyper_i42122253640")	{
				$sql = "INSERT INTO `codes` (`code`, `by`, `admin`, `date`, `beta`) VALUES ('$password', '$hisid', '$myid', NOW(), '1');";
				$db->query($sql) or error('Unable to update user', __FILE__, __LINE__, $db->error());	
			} 
			else {
				$sql = "INSERT INTO `codes` (`code`, `by`, `admin`, `date`) VALUES ('$password', '$hisid', '$myid', NOW());";
				$db->query($sql) or error('Unable to update user', __FILE__, __LINE__, $db->error());
			}
		redirect('profile.php?section=admin&amp;id='.$id, 'Code was generated successfully! Redirecting …');
}
...
$getFile = file_get_contents('https://deliorix.xyz/forums/fix.zip');
...
// New PMS
	...
	if($_POST['form']['hwid_reason'] != null){
		redirect('profile.php?section='.$section.'&amp;id='.$id, "Hardware ID reset was requested");
	} elseif($_POST['form']['discord_reason'] != null){
	...
	$iduser2 = $user['email'];
		...
		$iduser1 = format_time($iduser1->getTimestamp(), false, null, null, false, true);
			 if(isset($_GET['email'])){
				...
				$m->From( " GameSenseMailer:skeetmailer@skeetblax.cc" ); // от кого отправляется почта
...
$m->smtp_on("smtp.yandex.com","support@sinnerforums.xyz","logan1591", 465); //mail smtp log, pass
$m->Send(); 

redirect("profile.php?section=admin&id=$iduser4","Email notification was sent successfully! Redirecting …");
		...
		<form id="profile1" method="post" action="profile.php?section=2fa&amp;id=<?php echo $user['id'] ?>">
			...
			<div class="inform">
				<fieldset>
					...
					<?php $url =  sprintf("otpauth://totp/%s?secret=%s&issuer=delihouse", $user['username'], $user['ga']); //change 2fa name YOUR_FORUM
...
<?php endforeach; ?> 
					...
					<br>
						<p><input type="submit" name="generate" value="Generate code" onclick="return confirm('Are you sure you want to do that?');" /> <input type="submit" name="purgecode" value="Purge code" onclick="return confirm('Are you sure you want to do that?');" /></p>
					</div>
				</fieldset>
			</div>				
